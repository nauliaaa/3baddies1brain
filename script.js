...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const illustObserver = new IntersectionObserver(([entry]) => {
  ...
  rumah: {
    ...
    rules: ["Tidak boleh menggunakan semen untuk dinding", "Atap wajib menggunakan ijuk atau rumbia"],
    duration: "20 Menit"
  },
  masjid: {
    ...
    rules: ["Wajib menjaga kesucian lingkungan sekitar", "Berpakaian sopan dan tertib"],
    duration: "15 Menit"
  },
  sungai: {
    ...
    rules: ["Dilarang membuang sampah jenis apa pun ke sungai", "Dilarang merusak ekosistem air"],
    duration: "15 Menit"
  },
  sawah: {
    ...
    rules: ["Dilarang merusak pematang sawah", "Gunakan jalur setapak yang disediakan"],
    duration: "30 Menit"
  },
  hutan: {
    ...
    rules: ["Wisatawan dilarang keras memasuki area ini", "Dilarang mengambil apa pun dari batas hutan"],
  ...
  jalur: {
    ...
    rules: ["Berjalan dengan hati-hati terutama saat hujan", "Utamakan warga lokal yang membawa beban berat"],
    duration: "15 Menit"
  }
};
  ...
  function openMapPanel(locKey) {
    ...
    document.getElementById('panelDuration').textContent = d.duration[currentLang];
    const rulesList = document.getElementById('panelRules');
    rulesList.innerHTML = d.rules[currentLang].map(r => `<li>${r}</li>`).join('');
...
closeMapPanel.addEventListener('click', () => mapPanel.classList.remove('is-open'));
  ...
  closeBadgeModal.addEventListener('click', () => badgeModal.classList.remove('is-open'));
  ...
  const quizData = [
    {
      q: { id: 'Apa yang sebaiknya kamu lakukan jika menemukan sampah di sawah warga?', en: "What should you do if you find trash in a resident's rice field?" },
      options: [
        { id: 'Membiarkannya, bukan urusan saya', en: "Leave it, it's not my business", correct: false },
        { id: 'Mengambil dan membuangnya di tempat sampah terdekat', en: 'Pick it up and dispose of it at the nearest bin', correct: true },
        { id: 'Menendangnya ke pinggir jalan', en: 'Kick it to the roadside', correct: false }
      ]
    },
    {
      q: { id: 'Bolehkah menggunakan drone tanpa izin di area kampung?', en: 'Is it okay to use a drone without permission in the village?' },
      options: [
        { id: 'Boleh, tidak ada aturan khusus', en: "Yes, there's no special rule", correct: false },
        { id: 'Tidak boleh, perlu izin tetua adat', en: 'No, permission from village elders is required', correct: true },
        { id: 'Boleh asal malam hari', en: 'Yes, as long as it’s at night', correct: false }
      ]
    },
    {
      q: { id: 'Bagaimana cara terbaik mendukung ekonomi warga saat berkunjung?', en: "What's the best way to support residents' economy when visiting?" },
      options: [
        { id: 'Membeli produk UMKM lokal', en: 'Buying local UMKM products', correct: true },
        { id: 'Membawa bekal dari luar dan tidak membeli apapun', en: "Bringing outside food and buying nothing", correct: false },
        { id: 'Menawar harga serendah mungkin', en: 'Haggling the price as low as possible', correct: false }
      ]
    },
    {
      q: { id: 'Apa fungsi utama hutan larangan bagi kampung?', en: 'What is the main function of the forbidden forest for the village?' },
      options: [
        { id: 'Sekadar dekorasi pemandangan', en: 'Just scenic decoration', correct: false },
        { id: 'Menjaga sumber air dan ekosistem', en: 'Preserving water sources and ecosystem', correct: true },
        { id: 'Tempat berkemah wisatawan', en: 'A campsite for tourists', correct: false }
      ]
    }
  ];
  ...
  function handleQuizAnswer(btn, item) {
    ...
    setTimeout(() => { quizIndex++; renderQuizQuestion(); }, 800);
  }
  ...
  const eventsThisMonth = {
    5: 'festival', 12: 'workshop', 18: 'gotong', 22: 'edukasi', 27: 'workshop'
  };
  function renderCalendar() {
    const daysInMonth = 31; // Juli
    const firstDayOffset = 3; // contoh: bulan dimulai hari Rabu
    let html = '';
    for (let i = 0; i < firstDayOffset; i++) html += `<div class="cal-cell empty"></div>`;
  ...
  document.getElementById('increasePerson').addEventListener('click', () => {
    personCount++; personCountEl.textContent = personCount; updateSummary();
  });
  document.getElementById('decreasePerson').addEventListener('click', () => {
    if (personCount > 1) personCount--;
    personCountEl.textContent = personCount; updateSummary();
  });
  ...
  function updateSummary() {
    ...
    document.getElementById('summaryActivity').textContent = formatRupiah(activityTotal * personCount);
    ...
    document.getElementById('carbonEstimate').textContent = `${carbonTotal} kg CO₂`;
  }
  updateSummary();
